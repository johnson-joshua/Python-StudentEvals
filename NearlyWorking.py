...
df2 = pd.DataFrame(columns=["a", "b", "c", "faculty code", "semester", "name", "division"]);
resp = 0
totalResp = 0
numStudents = 0
sa = 0
...
def addToDataFrame(qNum, answer, numResponses):
    ...
    for response in range(int(numResponses)):
        ...
        df2.at[exportRow, int(qNum)] = intAnswer
        #df2.loc[exportRow] = 1
        exportRow += 1
...
#            print('A: ' +str(a))
        if(row['Answer'] == 'Disagree'):
            d = row['# Responses']
            addToDataFrame(row['Q #'], row['Answer'], d)
#            print('D: ' +str(d))
        if(row['Answer'] == 'Strongly Disagree'):
            sd = row['# Responses']
            addToDataFrame(row['Q #'], row['Answer'], sd)
#            print('SD: ' +str(sd))
        if(row['Answer'] == 'Not Applicable'):
            na = row['# Responses']
