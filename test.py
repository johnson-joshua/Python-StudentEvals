...
def addToDataFrame(qNum, answer, numResponses):
    if numResponses != 0:
        ...
        if str(answer) == 'Strongly Agree':
            intAnswer = 1
        elif str(answer) == 'Agree':
            intAnswer = 2
        elif str(answer) == 'Disagree':
            intAnswer = 3
        elif str(answer) == 'Strongly Disagree':
            intAnswer = 4
        elif str(answer) == 'Not Applicable':
            intAnswer = 5
        
        for response in range(int(numResponses)):
        #print('Response = ' +str(response))
            print('exportRow = ' +str(exportRow))
            #df2 = df2.assign(str(qNum) = intAnswer)
            df2.at[response, qNum] = intAnswer
        #df2.loc[response, exportRow] = intAnswer
